<script lang="ts">
	...
	type Product = 'perfusor' | 'physiology';
	...
	const products = [
		{ value: 'perfusor', label: 'Perfusoren', shortLabel: 'Perfusor' },
		{ value: 'physiology', label: 'Physiologische Parameter', shortLabel: 'Parameter' },
	] satisfies Array<{ value: Product; label: string; shortLabel: string }>;
	...
	let doseInputs = $state<Record<string, string>>(getDefaultDoseInputs(patient.group));
	...
	function formatMlRange(value: { lower: number; upper: number } | null) {
		...
		return `${numberFormatter.format(value.lower)}-${numberFormatter.format(value.upper)} mL`;
	}

	function getProductTitle(product: Product) {
		return product === 'perfusor' ? 'Perfusoren Rechner' : 'Physiologische Parameter';
	}
					...
					<Card.Content>
						...
						<div class="grid gap-3 md:grid-cols-3">
							<div class="rounded-xl border border-border bg-background/70 p-5">
								<p class="text-sm font-medium text-muted-foreground">Ideales Körpergewicht</p>
								<p class="mt-3 text-3xl font-semibold tabular-nums">{formatKg(idealBodyWeight)}</p>
							</div>
							...
							<div class="rounded-xl border border-border bg-background/70 p-5">
								...
								<p class="mt-3 text-3xl font-semibold tabular-nums">{formatLiters(bloodVolume)}</p>
							</div>
							...
							<div class="rounded-xl border border-border bg-background/70 p-5">
								...
								<p class="mt-3 text-3xl font-semibold tabular-nums">{formatMlRange(tidalVolume)}</p>
							</div>
						</div>
					</Card.Content>
				</Card.Root>
...
<nav class="fixed inset-x-0 bottom-0 z-50 px-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
	<div
		class="mx-auto grid max-w-md grid-cols-2 gap-1 rounded-[2rem] border border-border/80 bg-background/90 p-1 shadow-lg backdrop-blur"
		aria-label="Produkte"
	>
		{#each products as product}
			<Button
				...
				aria-current={activeProduct === product.value ? 'page' : undefined}
				onclick={() => {
					activeProduct = product.value;
				}}
			>
				<span class="hidden sm:inline">{product.label}</span>
				<span class="sm:hidden">{product.shortLabel}</span>
			</Button>
		{/each}
	</div>
</nav>
